...
import type { YatraContent } from '@/sanity/lib/content';
import { Flag, MapPin, Calendar, Heart, Shield, CheckCircle } from 'lucide-react';

export default function YatraPageClient({ yatraCampaign }: { yatraCampaign: YatraContent }) {
  ...
  return (
    <div className="space-y-8 sm:space-y-14 pb-20 md:pb-16">
      ...
      {/* 4. YATRA PHOTO GALLERY */}
      {yatraCampaign.gallery.length > 0 && (
              ...
              <div className="flex flex-wrap items-end justify-between gap-3 border-b border-road/20 pb-4">
                <div className="space-y-1">
                  ...
                  <p className="font-body text-sm text-ink/70 max-w-xl">
                    {yatraCampaign.distance} across India, captured along the way. Select any photo
                ...
                <span className="font-mono text-xs font-bold text-road bg-road/10 border border-road/20 px-3 py-1 rounded-full uppercase shrink-0">
                  {yatraCampaign.gallery.length} photos
                </span>
              </div>

              <PhotoGallery photos={yatraCampaign.gallery} label="Sangati Yatra photo gallery" />
            </div>
          </ScrollReveal>
        </section>
      )}
