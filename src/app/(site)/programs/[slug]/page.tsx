...
import { getProgram, getProgramSlugs } from '@/sanity/lib/content';
...
/** Programmes added in the admin panel after a deploy are rendered on first visit. */
      ...
      <section className="max-w-5xl mx-auto px-4 space-y-8 sm:space-y-10">
        ...
        <ScrollReveal variant="fade-up">
          <div className="bg-white border border-road/20 rounded-3xl p-6 sm:p-10 space-y-6 shadow-sm">
            ...
            <div className="text-lg sm:text-2xl font-body text-ink leading-relaxed italic border-l-4 border-marigold pl-4 sm:pl-6 py-2 space-y-4">
              <RichText value={program.body} fallback={[program.fullContent]} />
            </div>
        ...
        {program.gallery && program.gallery.length > 0 && (
          <ScrollReveal variant="fade-up" delay={150}>
            <div className="space-y-5 bg-mist/30 border border-road/20 rounded-3xl p-4 sm:p-6 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-road/20 pb-3 px-1">
                <div className="flex items-center gap-2">
                  <Camera className="w-5 h-5 text-road" aria-hidden="true" />
                ...
                <span className="font-mono text-xs font-bold text-road bg-road/10 border border-road/20 px-3 py-1 rounded-full uppercase">
                  {program.gallery.length} photo{program.gallery.length === 1 ? '' : 's'}
                </span>
              </div>

              <PhotoGallery
                photos={program.gallery}
                label={`${program.title} photo gallery`}
              />
            </div>
          </ScrollReveal>
        )}
